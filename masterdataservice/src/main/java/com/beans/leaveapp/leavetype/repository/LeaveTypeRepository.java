...
public interface LeaveTypeRepository extends CrudRepository<LeaveType, Integer>{
	 ...
	 @Query("select l from LeaveType l where employeeType.id in (select employeeType.id from Employee e where e.id = ?) and name not in ('Annual','Time-In-Lieu') and isDeleted=0")
	 List<LeaveType> getAllLeaveTypesGivenByEmployeeType(int employeeId);
}

...
public class EmployeeRepositoryImpl extends JdbcDaoSupport implements EmployeeRepositoryCustom {
	...
	public List<Employee> findAllEmployeesForSendingMonthlyLeaveReport() {
		return getJdbcTemplate().query("SELECT * FROM beans.Employee where employeeTypeId in (select id from beans.EmployeeType) and  userId not in(select userId from beans.UserToRole where userRoleId in (select id from beans.Role where role in('ROLE_DIR','ROLE_ADMIN'))) order by id",new RowMapper<Employee>(){
			...
			public Employee mapRow(ResultSet rs, int rowNum) throws SQLException {
				 Employee employeeBean=new Employee();  
			        ...
			        employeeBean.setMaritalStatus(rs.getString("maritalStatus"));
			        return employeeBean;  
			}
	});
	}
